...
class UIPersonnel_SquadBarracks extends UIPersonnel config(SquadManager);
...
{
	return class'XComGameState_LWSquadManager'.static.GetSquadManager().Squads.Find('ObjectID', SquadRef.ObjectID);
}
...
simulated function XComGameState_LWPersistentSquad GetCurrentSquad()
{
	...
	// Rai  - Adding another update here
...
simulated function bool CanTransferSoldier(StateObjectReference UnitRef, optional XComGameState_LWPersistentSquad SquadState)
{
	...
	if(SquadState != none)
	{
		...
		if(SquadState.IsDeployedOnMission() || SquadState.CurrentMission.ObjectID > 0)
...
simulated function UpdateSquadHeader()
{
	...
	{
		if (bSelectSquad && SquadState.IsDeployedOnMission())
			SelectOrViewBtn.SetDisabled(true);
		else
...
simulated function UpdateList()
{
	...
	for(i = 0; i < m_kList.itemCount; ++i)
	{
		...
		if(SquadState != none && SquadState.IsDeployedOnMission() && !SquadState.IsSoldierOnMission(UnitItem.UnitRef))
		{
...
simulated function OnDeleteSquadDialogCallback(Name eAction)
{
	...
	{
		SquadRef = UISquadListItem(m_kSquadList.GetItem(CurrentSquadSelection)).SquadRef;
		class'XComGameState_LWSquadManager'.static.GetSquadManager().RemoveSquadByRef(SquadRef);
		CurrentSquadSelection = -1;
		RefreshAllData();
	}
}
...
{
	class'XComGameState_LWSquadManager'.static.GetSquadManager().CreateEmptySquad();
...
function OnEditOrSelectClicked(UIButton Button)
{
	...
	if(bSelectSquad)
	{
		if(SquadState != none && !SquadState.IsDeployedOnMission())
		{
...
simulated function CloseScreen()
{
	...
	SquadSelect = class'XComGameState_LWSquadManager'.static.GetSquadManager().GetSquadSelect();
	if (SquadSelect != none)
	{
		SquadContainer = UISquadContainer(SquadSelect.GetChildByName('SquadSelect_SquadContainer_LW', false));
	}
