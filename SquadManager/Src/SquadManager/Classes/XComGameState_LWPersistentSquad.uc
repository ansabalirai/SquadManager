...
//---------------------------------------------------------------------------------------
class XComGameState_LWPersistentSquad extends XComGameState_GeoscapeEntity config(SquadManager);
...
function InitInfiltration( StateObjectReference MissionRef)
{
...
// Rai - These functions are not relevant for us in vanilla, so will disable them in bulk and see if we need to enable some functionality from them piece by piece
/*
...
static function bool IsUnitOnMission(XComGameState_Unit UnitState)
{
	switch (UnitState.GetMyTemplateName())
	{
		case  'SparkSoldier':
			...
			if (class'XComGameState_LWSquadManager'.static.GetSquadManager().UnitIsOnMission(UnitState.GetReference()))
			{
				return true;
			}
			break;
